<template>
	<z-paging ref="paging" v-model="content" @query="getUserArticle">
		...
		<view style="position: relative;">
			<view style="position: relative;">
				<image :src="profile.longtext.background_img" mode="aspectFill" style="width: 100%;height: 400rpx;">
				</image>
				<view
					style="border-radius: 6rpx; position: absolute;right: 0;bottom: 80rpx;background-color: rgba(0, 0, 0, 0.4);"
					class="tn-margin tn-padding-xs">
					<text class="tn-text-sm tn-color-white">IP属地：幻想乡</text>
				</view>
			</view>
			<view class="tn-bg-white" style="border-radius: 40rpx;position: absolute;bottom: -100rpx;width: 100%;">
				<view class="tn-margin" style="position: relative;top: -100rpx;">
					<tn-avatar :src="profile.head_img" size="xl" :borderSize="6" border borderColor="white"></tn-avatar>
				</view>
				<!-- 浮动样式 虽然很丑 但确实是这么写的 -->
				<view style="position: absolute;top: 0;width: 100%;">
									...
									<tn-button backgroundColor="#29B7CB" fontColor="tn-color-white">
										<view class="tn-flex tn-flex-col-center tn-flex-row-between"
											...
											<text class="tn-text-center"
												style="margin-left: auto;margin-right: auto;">关注</text>
						...
						<view class="tn-margin-top-lg">
							<view class="tn-flex tn-flex-direction-column">
								...
								<view class="tn-flex tn-margin-top tn-flex-top tn-color-gray--dark">
									...
									<text class="tn-text-ellipsis-2">{{profile.description}}</text>
								</view>
							</view>
						</view>
						<view class="tn-margin-top-xl tn-margin-bottom-lg">
							<view class="tn-flex tn-flex-col-center">
								<view class="tn-flex tn-flex-col-center tn-margin-right">
									<text
										class="tn-text-bold tn-text-xl tn-margin-right-xs">{{profile.expand.fansCount}}</text>
								...
								<view class="tn-flex tn-flex-col-center tn-margin-right">
									<text
										class="tn-text-bold tn-text-xl tn-margin-right-xs">{{profile.expand.likeCount}}</text>
					...
					<!-- 用户属性结束 -->
					<view>
						<v-tabs v-model="tabsIndex" :tabs="tabs" @change="changeTab" lineHeight="8rpx"
							lineColor="#29B7CB" activeColor="#29B7CB" :lineScale="0.2" color="#AAA"
							:scroll="false"></v-tabs>
						<swiper class="swiper" :current="tabsIndex" @animationfinish="swiperChange"
							style="height: 100vh;">
							<swiper-item style="overflow: auto;">
								<block v-for="(item,index) in content" :key="index">
									<view class="tn-margin">
										<view
											class="tn-flex tn-flex-col-center tn-flex-row-between tn-color-gray tn-text-xs">
											<view class="tn-flex tn-flex-col-center">
												<text
										...
										<view class="tn-margin-top-xs">
											<view @tap="goAticle(index)">
												...
												<!-- 复数开始 -->
												<view
													v-if="item.expand.images.length===2 || item.expand.images.length===4">
													<tn-grid align="left" :col="item.expand.images.length"
														hoverClass="none">
														<block v-for="(images, index) in item.expand.images"
															:key="index">
															<!-- H5 -->
															<!-- #ifndef MP-WEIXIN -->
															<tn-grid-item
												...
												<view
													v-if="item.expand.images.length===3|| item.expand.images.length>4">
												...
												<view>
													...
													<view
														...
														<view class="tn-flex tn-flex-col-center"
															@tap.stop="showComments(index)">
															<text class="tn-text-xxl tn-icon-comment"></text>
															<text
																class="tn-margin-left-xs">{{item.expand.comments.count}}</text>
														</view>
														<view class="tn-flex tn-flex-col-center"
															...
															<text
																class="tn-margin-left-xs">{{item.expand.like.likes_count}}</text>
														</view>
													</view>
												</view>
											</view>
